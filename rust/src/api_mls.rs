...
pub fn create_identity(
    ...
    let result = rt.block_on(async {
        ...
        let _store = store
            .as_mut()
            .ok_or_else(|| format_err!("<mls api fn[create_identity]> Can not get store err."))?;
...
pub fn create_key_package(
    ...
    let result = rt.block_on(async {
        ...
        let _store = store.as_mut().ok_or_else(|| {
...
pub fn group_create_config() -> Result<MlsGroupCreateConfig> {
    ...
    let result = rt.block_on(async {
        ...
        let _store = store.as_mut().ok_or_else(|| {
            format_err!("<mls api fn[group_create_config]> Can not get store err.")
        })?;
...
pub fn add_member(
    ...
    let result = rt.block_on(async {
        ...
        let _store = store.as_mut().ok_or_else(|| {
            format_err!("<mls api fn[add_member_to_group]> Can not get store err.")
        })?;
