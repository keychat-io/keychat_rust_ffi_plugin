...
async fn main() {
    ...
    let r1 = api::init_db(DB_PATH.to_string(), Some(words.to_owned()), false).await;
    info!("init_db {}: {:?}", DB_PATH, r1);

    let start = std::time::Instant::now();
    let r2 = api::init_cashu(32).await;
    ...
    if r2.is_ok()
        && api::get_mints()
            ...
            .unwrap_or(false)
    {
        let try_get_wallet = api::prepare_one_proofs(0, MINT_URL.to_string()).await;
        info!(
    ...
    // info!("add_mint: {:?}", api::add_mint(MINT_URL.to_string()).await);

    // test update restore
    // #[rustfmt::skip]
    // info!("{} restore: {:?}", mi.pubkey(), api::restore(MINT_URL.to_string(), Some(words.to_string()),1).await);
    // #[rustfmt::skip]
    // info!("{} restore: {:?}", migen.pubkey(), api::restore(MINT_URL.to_string(), Some(migen_words.to_string()),1).await);

    // test update mnemonic
    #[rustfmt::skip]
    let pubkey = api::get_mnemonic_info().await.unwrap().unwrap();
    #[rustfmt::skip]
    let _r2 = api::set_mnemonic(Some(migen_words.clone())).await.unwrap();
    #[rustfmt::skip]
    let pubkey2 =api::get_mnemonic_info().await.unwrap().unwrap();
    assert_eq!(pubkey, mi.pubkey());
    assert_eq!(pubkey2, migen.pubkey());
    api::set_mnemonic(None).await.unwrap();

    let r4 = api::get_mints().await;
    info!("get_mints: {:?}", r4);

    let r5 = api::get_balances().await;
    ...
    ".trim();
    let r6 = api::receive_token(encoded_token_8338.to_string()).await;
    ...
    ".trim();
    let r6 = api::receive_token(encoded_token_minibits.to_string()).await;
    info!("receive_token-minibits: {:?}", r6);

    info!("check_pending: {:?}", api::check_pending().await);

    info!("check_proofs: {:?}", api::check_proofs().await);

    let r5 = api::get_balances();
    info!("get_balances: {:?}", r5.await);

    let amount = 0;
    let s = api::prepare_one_proofs(amount, MINT_URL.to_string()).await;
    info!("prepare_one_proofs {}:{:?}\n", amount, s);

    let s = api::get_cashu_pending_transactions().await;
    info!("get_cashu_pending_transactions {}:{:?}\n", amount, s);
    let s = api::get_ln_pending_transactions().await;
    info!("get_ln_pending_transactions {}:{:?}\n", amount, s);
    // let amount = 1;
    // let s = api::send(amount, MINT_URL.to_string(), None).await.err();
    // info!("send {}:{:?}\n", amount, s);

    // let amount = 1;
    // let s = api::send_stamp(amount, vec![MINT_URL_MINIBITS.to_owned(), MINT_URL.to_owned()], None).await.err();
    // info!("send_stamp {}:{:?}\n", amount, s);

    // loop {
    //     let s = api::prepare_one_proofs(0, MINT_URL.to_string());
    //     info!("prepare_one_proofs 0:{:?}\n", s);
    //     let s = api::send(1, MINT_URL.to_string(), None).await.err();
    //     info!("send 1:{:?}\n", s,);
    //     if s.is_some() {
    //         break;
    //     }
    ...
    // let amount = 10;
    // let s = api::request_mint(amount, MINT_URL.to_string()).await;
    // info!("request_mint {}:{:?}\n", amount, s);

    // api::set_mnemonic(None).await.unwrap();
    // let amount = 10;
    // let tx = api::request_mint(amount, MINT_URL.to_string()).await.unwrap();
    // info!("request_mint {}:{:?}\n", amount, tx);

    // loop {
    //     std::thread::sleep_ms(10000);
    //     let tx = api::check_transaction(tx.id().to_owned()).await.unwrap();
    //     info!("mint {}:{:?}\n", amount, tx.status());
    ...
    // let s = api::mint_token(amount, hash.to_owned(), MINT_URL.to_string()).await;
    // info!("mint {}:{:?}\n", amount, s);
    ...
    // let s = api::melt(pr.to_owned(), MINT_URL.to_string(), None).await;
    // info!("melt: {:?}\n", s);
    ...
    info!(
        ...
        api::get_pending_transactions_count().await
    );

    // get_txs(3);
    info!(
        "remove_transactions:{:?}\n",
        api::remove_transactions(1696928517890, TransactionStatus::Success).await
    );
    get_txs(15).await;

    // info!("get_transactions_with_offset: {:?}", api::get_transactions_with_offset(0, 0).await);
}

async fn get_txs(page_limit: usize) {
    let mut txs = vec![];
    ...
    if page_limit > 0 {
        ...
        loop {
            ...
            match api::get_transactions_with_offset(offset, limit).await {
